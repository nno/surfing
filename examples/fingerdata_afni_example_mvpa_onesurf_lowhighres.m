...
% high-resolution surface that is downsampled to a low-res version. This
...
% number of iterations to reduce the number of nodes
% with 10 iterations the surface is reduced from 81914257
niter_lowres=10;
...
fnout_lowres_surface=sprintf('%s/ico%d_%sh_lowres_%diter',...
                        outdir,ld_high, hemi,niter_lowres);
...
% make a lower resolution version of the surface; this low-res surface
% defines the centers of the output surface dataset
% define high-res intermediate surface
[v_src,f_src]=surfing_subsample_surface(v,f,niter_lowres);

% find mapping from low to high resolution surface
low2high=surfing_maplow2hires(v_src', v');
...
%       the sixth argument is the mapping from low to high res
n2v=surfing_voxelselection(v', offsets, f', circledef, voldef, low2high);
...
surfing_write([fnout '.niml.dset'],S);

% write low-res surface
surfing_write([fnout_lowres_surface '.asc'],v_src,f_src);
